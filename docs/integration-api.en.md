...
The separately supplied name and email are used to pseudonymize extracted CV text once, before any live AI analysis. Laravel stores the canonical result encrypted, together with the pseudonymizer version and processing time. Screening, scoring and retries use that same copy without reading or transmitting raw extracted text again. A preparation failure stops processing before any provider call. The original PDF remains private, and the pseudonymized copy is removed with it under the CV retention policy.
