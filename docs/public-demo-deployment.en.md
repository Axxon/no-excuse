...
Run `make demo-prod-deploy`, then point an existing HTTPS reverse proxy to `127.0.0.1:${DEMO_HTTP_PORT}`. `APP_URL` is required and must contain the public HTTPS origin. Verify `https://demo.example.com/api/demo` and `make demo-prod-ps`.
...
The profile runs PostgreSQL, Redis, API, queue workers, scheduler and static frontend without exposing internal service ports. Update by pulling the source or downloading a version archive and rerunning the deploy target; do not delete volumes during a routine update.
