...
En mode IA `live`, le texte extrait est pseudonymisé par le service local `cv-pseudonymizer` avant tout appel fournisseur. Ce conteneur n'expose aucun port, n'a pas de sortie réseau, ne conserve aucun document et désactive les journaux d'accès. Laravel bloque l'appel distant si le service est indisponible ou retourne un résultat invalide. Ne journalisez jamais les corps de requête entre Laravel et ce service.
