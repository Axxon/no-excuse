...
const readOnly = computed(() => Boolean(auth.user?.organization?.is_demo))
...
<template>
  ...
  <section v-if="organization" class="page-section settings-layout">
    <form class="form-card" @submit.prevent="save">
      <div class="section-title"><span class="step-kicker">2 / 3</span><h2>{{ t('settings.company') }}</h2></div><p v-if="readOnly" class="locked-notice">🔒 {{ t('settings.demoLocked') }}</p><p v-if="error" class="alert">{{ error }}</p><p v-if="saved" class="success-line">{{ t('settings.saved') }}</p>
      <fieldset class="settings-fields" :disabled="readOnly">
      ...
      <button v-if="!readOnly" class="button">{{ t('common.save') }}</button>
      </fieldset>
    </form>
    <div class="form-card"><div class="section-title"><h2>{{ t('settings.team') }}</h2><button v-if="!readOnly && (auth.user?.role === 'owner' || auth.user?.role === 'admin')" class="button button-small button-ghost" @click="inviteOpen = !inviteOpen">+ {{ t('settings.add') }}</button></div><p>{{ readOnly ? t('settings.demoTeamLocked') : t('settings.teamLead') }}</p>
